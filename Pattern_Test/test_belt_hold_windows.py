"""
Belt Hold Pattern Test on Historical BTC Data - Windows Version
...
try:
    ...
    print("BeltHoldPattern imported successfully")
except ImportError as e:
    print(f"Error importing BeltHoldPattern: {e}")
    sys.exit(1)
...
class BeltHoldPatternTester:
    def __init__(self):
        ...
        self.detector = BeltHoldPattern()
        self.results = []
        print(f"BeltHoldPatternTester initialized")

    def _clear_old_charts(self):
        if self.charts_dir.exists():
            chart_files = list(self.charts_dir.glob('belt_hold_*.png'))
    ...
    def test_candle_by_candle(self, df, timeframe='5min', start_from=100, window_size=20):
        ...
        for i in range(start_from, total_candles):
            ...
            if is_detected:
                ...
                print(f"\nBelt Hold #{len(detections)} detected!")
    ...
    def _plot_detection(self, df, detected_at_index, pattern_index, timeframe,
        ...
        try:
            ...
            ax.set_title(
                f'Belt Hold Pattern Detection - BTC/USDT {timeframe}\n'
            ...
            filename = f"belt_hold_{timeframe}_detect{detected_at_index}_pattern{pattern_index}_{timestamp_str}.png"
    ...
    def _plot_all_patterns(self, df, pattern_indices, timeframe):
        ...
        try:
            ...
            ax.set_title(
                f'All Belt Hold Patterns - BTC/USDT {timeframe}\n'
            ...
            ax.scatter([], [], color='purple', s=150, marker='v',
                      label=f'Belt Hold Pattern ({len(pattern_indices)} total)')
            ...
            filename = f"belt_hold_{timeframe}_ALL_PATTERNS_summary.png"
            filepath = self.charts_dir / filename
    ...
    def _save_results(self, timeframe):
        results_file = self.output_dir / f'belt_hold_detections_{timeframe}.json'
...
def main():
    print("="*80)
    print("Belt Hold Pattern Test")
    print("="*80)

    try:
        tester = BeltHoldPatternTester()
