"""
Dark Cloud Cover Pattern Test on Historical BTC Data - Windows Version

This script tests the Dark Cloud Cover pattern candle-by-candle on historical data.

Author: Claude Code
Date: 2025-10-26

How to run in PyCharm:
1. Right-click on file -> Run 'test_dark_cloud_cover_windows'
OR
1. Open Terminal in PyCharm
2. python Pattern_Test/test_dark_cloud_cover_windows.py
"""
...
try:
    ...
    print("DarkCloudCoverPattern imported successfully")
except ImportError as e:
    print(f"Error importing DarkCloudCoverPattern: {e}")
...
class DarkCloudCoverPatternTester:
    """Dark Cloud Cover pattern tester class for Windows"""
    ...
    def __init__(self):
        ...
        self.dark_cloud_cover_detector = DarkCloudCoverPattern()

        # Store results
        self.results = []

        print(f"DarkCloudCoverPatternTester initialized")
        print(f"   Data directory: {self.data_dir}")
        print(f"   Output directory: {self.output_dir}")
        print(f"   Charts directory: {self.charts_dir}")

    def _clear_old_charts(self):
        """Clear all previous Dark Cloud Cover charts"""
        if self.charts_dir.exists():
            chart_files = list(self.charts_dir.glob('dark_cloud_cover_*.png'))
            ...
            if chart_files:
                print(f"Cleared {len(chart_files)} previous Dark Cloud Cover charts")
    ...
    def test_candle_by_candle(self, df, timeframe='5min', start_from=100,
        ...
        - Detector searches in last 11 candles within that window
        ...
        print(f"Detector lookback: 11 candles (last 11 in window)")
        ...
        for i in range(start_from, total_candles):
            ...
            # Test pattern
            try:
                is_detected = self.dark_cloud_cover_detector.detect(df_slice)
            except Exception as e:
                if i == start_from:
            ...
            if is_detected:
                ...
                try:
                    pattern_info = self.dark_cloud_cover_detector.get_pattern_info(df_slice, timeframe)
                ...
                print(f"\nDark Cloud Cover pattern #{len(detections)} detected!")
                ...
                if pattern_info:
                    ...
                    # Dark Cloud Cover specific info
                    metadata = pattern_info.get('metadata', {})
                    if metadata:
                        print(f"   Penetration ratio: {metadata.get('penetration_ratio', 0):.2f}")
    ...
    def _plot_detection(self, df, detected_at_index, pattern_index, timeframe,
        ...
        try:
            ...
            ax.set_title(
                f'Dark Cloud Cover Pattern Detection - BTC/USDT {timeframe}\n'
            ...
            if pattern_info:
                ...
                info_text += f"Lookback: 11 candles\n"
                ...
                metadata = pattern_info.get('metadata', {})
                if metadata:
                    info_text += f"Penetration: {metadata.get('penetration_ratio', 0):.2f}\n"
            ...
            filename = f"dark_cloud_cover_{timeframe}_detect{detected_at_index}_pattern{pattern_index}_{timestamp_str}.png"
        ...
        try:
            ...
            for pattern_idx in pattern_indices:
                if start_idx <= pattern_idx < end_idx:
                    ...
                    # Dark Cloud Cover marker at top (downward triangle - bearish)
            ...
            ax.set_title(
                f'All Dark Cloud Cover Patterns - BTC/USDT {timeframe}\n'
            ...
            ax.scatter([], [], color='darkred', s=150, marker='v',
                      label=f'Dark Cloud Cover Pattern ({len(pattern_indices)} total)')
            ...
            filename = f"dark_cloud_cover_{timeframe}_ALL_PATTERNS_summary.png"
            filepath = self.charts_dir / filename
    ...
    def _save_results(self, timeframe):
        ...
        results_file = self.output_dir / f'dark_cloud_cover_detections_{timeframe}.json'
...
def main():
    ...
    print("="*80)
    print("Dark Cloud Cover Pattern Test on Historical BTC/USDT Data - Windows")
    print("="*80)

    try:
        # Create tester
        tester = DarkCloudCoverPatternTester()
