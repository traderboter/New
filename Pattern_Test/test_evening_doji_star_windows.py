"""
Evening Doji Star Pattern Test on Historical BTC Data - Windows Version

This script tests the Evening Doji Star pattern candle-by-candle on historical data.

Author: Claude Code
Date: 2025-10-26

How to run in PyCharm:
1. Right-click on file -> Run 'test_evening_doji_star_windows'
OR
1. Open Terminal in PyCharm
2. python Pattern_Test/test_evening_doji_star_windows.py
"""
...
try:
    ...
    print("EveningDojiStarPattern imported successfully")
except ImportError as e:
    print(f"Error importing EveningDojiStarPattern: {e}")
...
class EveningDojiStarPatternTester:
    """Evening Doji Star pattern tester class for Windows"""
    ...
    def __init__(self):
        ...
        self.evening_doji_star_detector = EveningDojiStarPattern()

        # Store results
        self.results = []

        print(f"EveningDojiStarPatternTester initialized")
        print(f"   Data directory: {self.data_dir}")
        print(f"   Output directory: {self.output_dir}")
        print(f"   Charts directory: {self.charts_dir}")

    def _clear_old_charts(self):
        """Clear all previous Evening Doji Star charts"""
        if self.charts_dir.exists():
            chart_files = list(self.charts_dir.glob('evening_doji_star_*.png'))
            ...
            if chart_files:
                print(f"Cleared {len(chart_files)} previous Evening Doji Star charts")
    ...
    def test_candle_by_candle(self, df, timeframe='5min', start_from=100,
        ...
        - Detector searches in last 12 candles within that window
        ...
        print(f"Detector lookback: 12 candles (last 12 in window)")
        print(f"TA-Lib requirement: minimum 13 candles")
        print(f"Pattern: 3-candle bearish reversal (Bullish → Doji → Bearish)")
        ...
        for i in range(start_from, total_candles):
            ...
            # Test pattern
            try:
                is_detected = self.evening_doji_star_detector.detect(df_slice)
            except Exception as e:
                if i == start_from:
            ...
            if is_detected:
                ...
                try:
                    pattern_info = self.evening_doji_star_detector.get_pattern_info(df_slice, timeframe)
                    candles_ago = pattern_info.get('candles_ago', 0)
                except Exception as e:
                    pattern_info = None
                    candles_ago = 0

                # Calculate the actual index of the pattern candle (3rd candle of pattern)
                ...
                print(f"\nEvening Doji Star pattern #{len(detections)} detected!")
                ...
                if pattern_info:
                    ...
                    # Evening Doji Star specific info
                    metadata = pattern_info.get('metadata', {})
                    if metadata:
                        print(f"   Doji ratio: {metadata.get('doji_ratio', 0):.2f}")
    ...
    def _plot_detection(self, df, detected_at_index, pattern_index, timeframe,
        ...
        Args:
            ...
            pattern_index: The actual candle index where the pattern ends (3rd candle)
        ...
        try:
            ...
            # Mark all patterns in this window (mark the 3rd candle and the 3-candle sequence)
            if patterns_in_range:
                for det in patterns_in_range:
                    det_idx = det['index']  # This is the 3rd candle

                    # Check if we have all 3 candles in the window
                    if det_idx - 2 >= window_start_idx:
                        # Calculate positions for all 3 candles
                        first_pos = det_idx - 2 - window_start_idx
                        doji_pos = det_idx - 1 - window_start_idx
                        third_pos = det_idx - window_start_idx

                        # Get the 3 candles
                        first_candle = df[det_idx - 2]
                        doji_candle = df[det_idx - 1]
                        third_candle = df[det_idx]

                        if det_idx == pattern_index:
                            # Main pattern - mark all 3 candles
                            # First candle (bullish) - green circle
                            ax.scatter([first_pos], [first_candle['high']],
                                      color='green', s=200, marker='o', zorder=5,
                                      edgecolors='darkgreen', linewidths=2,
                                      label=f'1st: Bullish (candle {det_idx-2})')

                            # Doji candle - blue star
                            ax.scatter([doji_pos], [doji_candle['high']],
                                      color='blue', s=250, marker='*', zorder=5,
                                      edgecolors='darkblue', linewidths=2,
                                      label=f'2nd: Doji (candle {det_idx-1})')

                            # Third candle (bearish) - red triangle down
                            ax.scatter([third_pos], [third_candle['high']],
                                      color='darkred', s=250, marker='v', zorder=5,
                                      edgecolors='red', linewidths=2,
                                      label=f'3rd: Bearish (candle {det_idx})')
                        else:
                            # Other patterns - smaller, transparent
                            ax.scatter([third_pos], [third_candle['high']],
            ...
            ax.set_title(
                f'Evening Doji Star Pattern Detection - BTC/USDT {timeframe}\n'
                f'Main Pattern: Candle #{pattern_index} at {pattern_candle["timestamp"]}\n'
                f'3-Candle Sequence: Bullish (○) → Doji (★) → Bearish (▼)\n'
            ...
            if pattern_info:
                ...
                info_text += f"Lookback: 12 candles\n"
                info_text += f"TA-Lib minimum: 13 candles\n"
                info_text += f"NO future data!\n\n"
                info_text += f"PATTERN INFO:\n"
                info_text += f"Type: 3-Candle Bearish Reversal\n"
                ...
                metadata = pattern_info.get('metadata', {})
                if metadata:
                    info_text += f"Doji ratio: {metadata.get('doji_ratio', 0):.2f}\n"
            ...
            filename = f"evening_doji_star_{timeframe}_detect{detected_at_index}_pattern{pattern_index}_{timestamp_str}.png"
    ...
    def _plot_all_patterns(self, df, pattern_indices, timeframe):
        ...
        try:
            ...
            # Mark all pattern candles (mark the 3rd candle of each pattern)
            for pattern_idx in pattern_indices:
                if start_idx <= pattern_idx < end_idx:
                    pattern_position = pattern_idx - start_idx
                    pattern_candle = df[pattern_idx]

                    # Evening Doji Star marker at top (downward triangle - bearish)
            ...
            ax.set_title(
                f'All Evening Doji Star Patterns - BTC/USDT {timeframe}\n'
                f'Total {len(pattern_indices)} patterns from candle {min_idx} to {max_idx}\n'
                f'3-Candle Bearish Reversal: Bullish → Doji → Bearish',
                fontsize=16,
                fontweight='bold'
            )
            ...
            ax.scatter([], [], color='darkred', s=150, marker='v',
                      label=f'Evening Doji Star ({len(pattern_indices)} total)')
            ax.legend(loc='upper left', fontsize=12)

            # Add info text
            info_text = f"Patterns: {len(pattern_indices)}\n"
            info_text += f"Type: 3-Candle Bearish\n"
            ...
            filename = f"evening_doji_star_{timeframe}_ALL_PATTERNS_summary.png"
            filepath = self.charts_dir / filename
    ...
    def _save_results(self, timeframe):
        ...
        results_file = self.output_dir / f'evening_doji_star_detections_{timeframe}.json'
...
def main():
    ...
    print("="*80)
    print("Evening Doji Star Pattern Test on Historical BTC/USDT Data - Windows")
    print("Testing 3-candle bearish reversal: Bullish → Doji → Bearish")
    print("="*80)

    try:
        # Create tester
        tester = EveningDojiStarPatternTester()
