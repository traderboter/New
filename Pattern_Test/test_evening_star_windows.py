"""
Evening Star Pattern Test on Historical BTC Data - Windows Version

This script tests the Evening Star pattern candle-by-candle on historical data.

Author: Claude Code
Date: 2025-10-26

How to run in PyCharm:
1. Right-click on file -> Run 'test_evening_star_windows'
OR
1. Open Terminal in PyCharm
2. python Pattern_Test/test_evening_star_windows.py
"""
...
try:
    ...
    print("EveningStarPattern imported successfully")
except ImportError as e:
    print(f"Error importing EveningStarPattern: {e}")
...
class EveningStarPatternTester:
    """Evening Star pattern tester class for Windows"""
    ...
    def __init__(self):
        ...
        self.evening_star_detector = EveningStarPattern()

        # Store results
        self.results = []

        print(f"EveningStarPatternTester initialized")
        print(f"   Data directory: {self.data_dir}")
        print(f"   Output directory: {self.output_dir}")
        print(f"   Charts directory: {self.charts_dir}")

    def _clear_old_charts(self):
        """Clear all previous Evening Star charts"""
        if self.charts_dir.exists():
            chart_files = list(self.charts_dir.glob('evening_star_*.png'))
            ...
            if chart_files:
                print(f"Cleared {len(chart_files)} previous Evening Star charts")
    ...
    def test_candle_by_candle(self, df, timeframe='5min', start_from=100,
        ...
        print(f"Pattern: 3-candle bearish reversal (Bullish → Small → Bearish)")
        ...
        for i in range(start_from, total_candles):
            ...
            # Test pattern
            try:
                is_detected = self.evening_star_detector.detect(df_slice)
            except Exception as e:
                if i == start_from:
            ...
            if is_detected:
                ...
                try:
                    pattern_info = self.evening_star_detector.get_pattern_info(df_slice, timeframe)
                ...
                print(f"\nEvening Star pattern #{len(detections)} detected!")
    ...
    def _plot_detection(self, df, detected_at_index, pattern_index, timeframe,
        ...
        try:
                ...
                for det in patterns_in_range:
                    ...
                    if det_idx - 2 >= window_start_idx:
                        ...
                        if det_idx == pattern_index:
                            ...
                            # Second candle - yellow star
                            ax.scatter([second_pos], [second_candle['high']],
                                      color='gold', s=250, marker='*', zorder=5,
                                      edgecolors='orange', linewidths=2,
                                      label=f'2nd: Small/Star (candle {det_idx-1})')
            ...
            ax.set_title(
                f'Evening Star Pattern Detection - BTC/USDT {timeframe}\n'
                f'Main Pattern: Candle #{pattern_index} at {pattern_candle["timestamp"]}\n'
                f'3-Candle Sequence: Bullish (O) -> Star (*) -> Bearish (v)\n'
            ...
            filename = f"evening_star_{timeframe}_detect{detected_at_index}_pattern{pattern_index}_{timestamp_str}.png"
        ...
        try:
            ...
            for pattern_idx in pattern_indices:
                if start_idx <= pattern_idx < end_idx:
                    ...
                    # Evening Star marker at top (downward triangle - bearish)
            ...
            ax.set_title(
                f'All Evening Star Patterns - BTC/USDT {timeframe}\n'
                f'Total {len(pattern_indices)} patterns from candle {min_idx} to {max_idx}\n'
                f'3-Candle Bearish Reversal: Bullish -> Star -> Bearish',
                fontsize=16,
                fontweight='bold'
            )
            ...
            ax.scatter([], [], color='darkred', s=150, marker='v',
                      label=f'Evening Star ({len(pattern_indices)} total)')
            ...
            filename = f"evening_star_{timeframe}_ALL_PATTERNS_summary.png"
            filepath = self.charts_dir / filename
    ...
    def _save_results(self, timeframe):
        ...
        results_file = self.output_dir / f'evening_star_detections_{timeframe}.json'
...
def main():
    ...
    print("="*80)
    print("Evening Star Pattern Test on Historical BTC/USDT Data - Windows")
    print("Testing 3-candle bearish reversal: Bullish -> Star -> Bearish")
    print("="*80)

    try:
        # Create tester
        tester = EveningStarPatternTester()
