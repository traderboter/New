"""
Gravestone Doji Pattern Test on Historical BTC Data - Windows Version
...
try:
    ...
    print("GravestoneDojiPattern imported successfully")
except ImportError as e:
    print(f"Error importing GravestoneDojiPattern: {e}")
    sys.exit(1)
...
class GravestoneDojiPatternTester:
    def __init__(self):
        ...
        self.detector = GravestoneDojiPattern()
        self.results = []
        print(f"GravestoneDojiPatternTester initialized")

    def _clear_old_charts(self):
        if self.charts_dir.exists():
            chart_files = list(self.charts_dir.glob('gravestone_doji_*.png'))
    ...
    def test_candle_by_candle(self, df, timeframe='5min', start_from=100, window_size=20):
        ...
        for i in range(start_from, total_candles):
            ...
            if is_detected:
                ...
                print(f"\nGravestone Doji #{len(detections)} detected!")
    ...
    def _plot_detection(self, df, detected_at_index, pattern_index, timeframe,
        ...
        try:
                ...
                for det in patterns_in_range:
                    ...
                    if det_idx == pattern_index:
                        ax.scatter([position], [candle['high']], color='red', s=250, marker='v', zorder=5,
                                  edgecolors='darkred' if 'red' != 'purple' else 'indigo', linewidths=2, label=f'Main Pattern (candle {det_idx})')
                    else:
                        secondary_color = 'cyan' if 'red' in ['blue', 'purple'] else 'orange'
            ...
            ax.set_title(
                f'Gravestone Doji Pattern Detection - BTC/USDT {timeframe}\n'
            ...
            filename = f"gravestone_doji_{timeframe}_detect{detected_at_index}_pattern{pattern_index}_{timestamp_str}.png"
    ...
    def _plot_all_patterns(self, df, pattern_indices, timeframe):
        ...
        try:
            ...
            ax.set_title(
                f'All Gravestone Doji Patterns - BTC/USDT {timeframe}\n'
            ...
            ax.scatter([], [], color='red', s=150, marker='v',
                      label=f'Gravestone Doji Pattern ({len(pattern_indices)} total)')
            ...
            filename = f"gravestone_doji_{timeframe}_ALL_PATTERNS_summary.png"
            filepath = self.charts_dir / filename
    ...
    def _save_results(self, timeframe):
        results_file = self.output_dir / f'gravestone_doji_detections_{timeframe}.json'
...
def main():
    print("="*80)
    print("Gravestone Doji Pattern Test")
    print("="*80)

    try:
        tester = GravestoneDojiPatternTester()
