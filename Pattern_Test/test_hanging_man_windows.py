"""
Hanging Man Pattern Test on Historical BTC Data - Windows Version
...
try:
    ...
    print("HangingManPattern imported successfully")
except ImportError as e:
    print(f"Error importing HangingManPattern: {e}")
    sys.exit(1)
...
class HangingManPatternTester:
    def __init__(self):
        ...
        self.detector = HangingManPattern()
        self.results = []
        print(f"HangingManPatternTester initialized")

    def _clear_old_charts(self):
        if self.charts_dir.exists():
            chart_files = list(self.charts_dir.glob('hanging_man_*.png'))
    ...
    def test_candle_by_candle(self, df, timeframe='5min', start_from=100, window_size=20):
        ...
        print(f"Lookback: 5, Min candles: 12")
        ...
        for i in range(start_from, total_candles):
            ...
            if is_detected:
                ...
                print(f"
Hanging Man #{len(detections)} detected!")
        ...
        print(f"\nTotal detections: {len(detections)}, Unique: {len(unique_patterns)}")        self.results.extend(detections)
...
Creating summary chart with all {len(unique_patterns)} unique patterns...")
    ...
    def _plot_detection(self, df, detected_at_index, pattern_index, timeframe,
        ...
        try:
            ...
            if patterns_in_range:
                for det in patterns_in_range:
                    ...
                    if det_idx == pattern_index:
                        ax.scatter([position], [candle['high']], color='red', s=250, marker='v', zorder=5,
                                  edgecolors='darkred', linewidths=2, label=f'Main Pattern (candle {det_idx})')
                    else:
                        ax.scatter([position], [candle['high']], color='cyan' if 'red' == 'blue' else 'orange',
            ...
            ax.set_title(
                f'Hanging Man Pattern Detection - BTC/USDT {timeframe}
'
...
"
            ...
            filename = f"hanging_man_{timeframe}_detect{detected_at_index}_pattern{pattern_index}_{timestamp_str}.png"
    ...
    def _plot_all_patterns(self, df, pattern_indices, timeframe):
        ...
        try:
            ...
            ax.set_title(
                f'All Hanging Man Patterns - BTC/USDT {timeframe}
'
            ...
            ax.scatter([], [], color='red', s=150, marker='v',
                      label=f'Hanging Man Pattern ({len(pattern_indices)} total)')
...
"
            ...
            filename = f"hanging_man_{timeframe}_ALL_PATTERNS_summary.png"
            filepath = self.charts_dir / filename
    ...
    def _save_results(self, timeframe):
        results_file = self.output_dir / f'hanging_man_detections_{timeframe}.json'
...
def main():
    print("="*80)
    print("Hanging Man Pattern Test")
    print("="*80)

    try:
        tester = HangingManPatternTester()
