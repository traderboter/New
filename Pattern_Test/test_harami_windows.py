"""
Harami Pattern Test on Historical BTC Data - Windows Version
...
try:
    ...
    print("HaramiPattern imported successfully")
except ImportError as e:
    print(f"Error importing HaramiPattern: {e}")
    sys.exit(1)
...
class HaramiPatternTester:
    def __init__(self):
        ...
        self.detector = HaramiPattern()
        self.results = []
        print(f"HaramiPatternTester initialized")

    def _clear_old_charts(self):
        if self.charts_dir.exists():
            chart_files = list(self.charts_dir.glob('harami_*.png'))
    ...
    def test_candle_by_candle(self, df, timeframe='5min', start_from=100, window_size=20):
        ...
        print(f"Testing both Bullish and Bearish Harami")
        ...
        for i in range(start_from, total_candles):
            ...
            if is_detected:
                ...
                print(f"
Harami #{len(detections)} detected!")
...
Creating summary chart with all {len(unique_patterns)} unique patterns...")
    ...
    def _plot_detection(self, df, detected_at_index, pattern_index, timeframe,
        ...
        try:
            ...
            ax.set_title(
                f'Harami Pattern Detection - BTC/USDT {timeframe}
'
...
"
            ...
            filename = f"harami_{timeframe}_detect{detected_at_index}_pattern{pattern_index}_{timestamp_str}.png"
    ...
    def _plot_all_patterns(self, df, pattern_indices, timeframe):
        ...
        try:
            ...
            ax.set_title(
                f'All Harami Patterns - BTC/USDT {timeframe}
'
            ...
            ax.scatter([], [], color='blue', s=150, marker='v',
                      label=f'Harami Pattern ({len(pattern_indices)} total)')
...
"
            ...
            filename = f"harami_{timeframe}_ALL_PATTERNS_summary.png"
            filepath = self.charts_dir / filename
    ...
    def _save_results(self, timeframe):
        results_file = self.output_dir / f'harami_detections_{timeframe}.json'
...
def main():
    print("="*80)
    print("Harami Pattern Test (Bullish & Bearish)")
    print("="*80)

    try:
        tester = HaramiPatternTester()
