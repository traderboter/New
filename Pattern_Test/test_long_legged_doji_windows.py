"""
Long-Legged Doji Pattern Test on Historical BTC Data - Windows Version
...
try:
    ...
    print("LongLeggedDojiPattern imported successfully")
except ImportError as e:
    print(f"Error importing LongLeggedDojiPattern: {e}")
    sys.exit(1)
...
class LongLeggedDojiPatternTester:
    def __init__(self):
        ...
        self.detector = LongLeggedDojiPattern()
        self.results = []
        print(f"LongLeggedDojiPatternTester initialized")

    def _clear_old_charts(self):
        if self.charts_dir.exists():
            chart_files = list(self.charts_dir.glob('long_legged_doji_*.png'))
    ...
    def test_candle_by_candle(self, df, timeframe='5min', start_from=100, window_size=20):
        ...
        for i in range(start_from, total_candles):
            ...
            if is_detected:
                ...
                print(f"\nLong-Legged Doji #{len(detections)} detected!")
    ...
    def _plot_detection(self, df, detected_at_index, pattern_index, timeframe,
        ...
        try:
            ...
            if patterns_in_range:
                for det in patterns_in_range:
                    ...
                    if det_idx == pattern_index:
                        ax.scatter([position], [candle['high']], color='orange', s=250, marker='v', zorder=5,
                                  edgecolors='darkorange' if 'orange' != 'purple' else 'indigo', linewidths=2, label=f'Main Pattern (candle {det_idx})')
                    else:
                        secondary_color = 'cyan' if 'orange' in ['blue', 'purple'] else 'orange'
            ...
            ax.set_title(
                f'Long-Legged Doji Pattern Detection - BTC/USDT {timeframe}\n'
            ...
            filename = f"long_legged_doji_{timeframe}_detect{detected_at_index}_pattern{pattern_index}_{timestamp_str}.png"
    ...
    def _plot_all_patterns(self, df, pattern_indices, timeframe):
        ...
        try:
            ...
            for pattern_idx in pattern_indices:
                if start_idx <= pattern_idx < end_idx:
                    ...
                    ax.scatter([pattern_position], [pattern_candle['high']],
                              color='orange', s=150, marker='v', zorder=5, alpha=0.7)
            ...
            ax.set_title(
                f'All Long-Legged Doji Patterns - BTC/USDT {timeframe}\n'
            ...
            ax.scatter([], [], color='orange', s=150, marker='v',
                      label=f'Long-Legged Doji Pattern ({len(pattern_indices)} total)')
            ...
            filename = f"long_legged_doji_{timeframe}_ALL_PATTERNS_summary.png"
            filepath = self.charts_dir / filename
    ...
    def _save_results(self, timeframe):
        results_file = self.output_dir / f'long_legged_doji_detections_{timeframe}.json'
...
def main():
    print("="*80)
    print("Long-Legged Doji Pattern Test")
    print("="*80)

    try:
        tester = LongLeggedDojiPatternTester()
