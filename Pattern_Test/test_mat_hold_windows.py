"""
Mat Hold Pattern Test on Historical BTC Data - Windows Version
...
try:
    ...
    print("MatHoldPattern imported successfully")
except ImportError as e:
    print(f"Error importing MatHoldPattern: {e}")
    sys.exit(1)
...
class MatHoldPatternTester:
    def __init__(self):
        ...
        self.detector = MatHoldPattern()
        self.results = []
        print(f"MatHoldPatternTester initialized")

    def _clear_old_charts(self):
        if self.charts_dir.exists():
            chart_files = list(self.charts_dir.glob('mat_hold_*.png'))
    ...
    def test_candle_by_candle(self, df, timeframe='5min', start_from=100, window_size=20):
        ...
        for i in range(start_from, total_candles):
            ...
            if is_detected:
                ...
                print(f"\nMat Hold #{len(detections)} detected!")
    ...
    def _plot_detection(self, df, detected_at_index, pattern_index, timeframe,
        ...
        try:
            ...
            ax.set_title(
                f'Mat Hold Pattern Detection - BTC/USDT {timeframe}\n'
            ...
            filename = f"mat_hold_{timeframe}_detect{detected_at_index}_pattern{pattern_index}_{timestamp_str}.png"
    ...
    def _plot_all_patterns(self, df, pattern_indices, timeframe):
        ...
        try:
            ...
            ax.set_title(
                f'All Mat Hold Patterns - BTC/USDT {timeframe}\n'
            ...
            ax.scatter([], [], color='blue', s=150, marker='v',
                      label=f'Mat Hold Pattern ({len(pattern_indices)} total)')
            ...
            filename = f"mat_hold_{timeframe}_ALL_PATTERNS_summary.png"
            filepath = self.charts_dir / filename
    ...
    def _save_results(self, timeframe):
        results_file = self.output_dir / f'mat_hold_detections_{timeframe}.json'
...
def main():
    print("="*80)
    print("Mat Hold Pattern Test")
    print("="*80)

    try:
        tester = MatHoldPatternTester()
