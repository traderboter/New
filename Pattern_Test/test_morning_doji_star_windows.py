"""
Morning Doji Star Pattern Test on Historical BTC Data - Windows Version
...
try:
    ...
    print("MorningDojiStarPattern imported successfully")
except ImportError as e:
    print(f"Error importing MorningDojiStarPattern: {e}")
    sys.exit(1)
...
class MorningDojiStarPatternTester:
    def __init__(self):
        ...
        self.detector = MorningDojiStarPattern()
        self.results = []
        print(f"MorningDojiStarPatternTester initialized")

    def _clear_old_charts(self):
        if self.charts_dir.exists():
            chart_files = list(self.charts_dir.glob('morning_doji_star_*.png'))
    ...
    def test_candle_by_candle(self, df, timeframe='5min', start_from=100, window_size=20):
        ...
        for i in range(start_from, total_candles):
            ...
            if is_detected:
                ...
                print(f"
Morning Doji Star #{len(detections)} detected!")
...
Creating summary chart with all {len(unique_patterns)} unique patterns...")
    ...
    def _plot_detection(self, df, detected_at_index, pattern_index, timeframe,
        ...
        try:
            ...
            ax.set_title(
                f'Morning Doji Star Pattern Detection - BTC/USDT {timeframe}
'
...
"
            ...
            filename = f"morning_doji_star_{timeframe}_detect{detected_at_index}_pattern{pattern_index}_{timestamp_str}.png"
    ...
    def _plot_all_patterns(self, df, pattern_indices, timeframe):
        ...
        try:
            ...
            ax.set_title(
                f'All Morning Doji Star Patterns - BTC/USDT {timeframe}
'
            ...
            ax.scatter([], [], color='blue', s=150, marker='v',
                      label=f'Morning Doji Star Pattern ({len(pattern_indices)} total)')
...
"
            ...
            filename = f"morning_doji_star_{timeframe}_ALL_PATTERNS_summary.png"
            filepath = self.charts_dir / filename
    ...
    def _save_results(self, timeframe):
        results_file = self.output_dir / f'morning_doji_star_detections_{timeframe}.json'
...
def main():
    print("="*80)
    print("Morning Doji Star Pattern Test")
    print("="*80)

    try:
        tester = MorningDojiStarPatternTester()
