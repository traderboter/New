"""
Piercing Line Pattern Test on Historical BTC Data - Windows Version
...
try:
    ...
    print("PiercingLinePattern imported successfully")
except ImportError as e:
    print(f"Error importing PiercingLinePattern: {e}")
    sys.exit(1)
...
class PiercingLinePatternTester:
    def __init__(self):
        ...
        self.detector = PiercingLinePattern()
        self.results = []
        print(f"PiercingLinePatternTester initialized")

    def _clear_old_charts(self):
        if self.charts_dir.exists():
            chart_files = list(self.charts_dir.glob('piercing_line_*.png'))
    ...
    def test_candle_by_candle(self, df, timeframe='5min', start_from=100, window_size=20):
        ...
        for i in range(start_from, total_candles):
            ...
            if is_detected:
                ...
                print(f"
Piercing Line #{len(detections)} detected!")
...
Creating summary chart with all {len(unique_patterns)} unique patterns...")
    ...
    def _plot_detection(self, df, detected_at_index, pattern_index, timeframe,
        ...
        try:
            ...
            ax.set_title(
                f'Piercing Line Pattern Detection - BTC/USDT {timeframe}
'
...
"
            ...
            filename = f"piercing_line_{timeframe}_detect{detected_at_index}_pattern{pattern_index}_{timestamp_str}.png"
    ...
    def _plot_all_patterns(self, df, pattern_indices, timeframe):
        ...
        try:
            ...
            ax.set_title(
                f'All Piercing Line Patterns - BTC/USDT {timeframe}
'
            ...
            ax.scatter([], [], color='blue', s=150, marker='v',
                      label=f'Piercing Line Pattern ({len(pattern_indices)} total)')
...
"
            ...
            filename = f"piercing_line_{timeframe}_ALL_PATTERNS_summary.png"
            filepath = self.charts_dir / filename
    ...
    def _save_results(self, timeframe):
        results_file = self.output_dir / f'piercing_line_detections_{timeframe}.json'
...
def main():
    print("="*80)
    print("Piercing Line Pattern Test")
    print("="*80)

    try:
        tester = PiercingLinePatternTester()
