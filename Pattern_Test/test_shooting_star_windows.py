"""
Shooting Star Pattern Test on Historical BTC Data - Windows Version
...
try:
    ...
    print("ShootingStarPattern imported successfully")
except ImportError as e:
    print(f"Error importing ShootingStarPattern: {e}")
    sys.exit(1)
...
class ShootingStarPatternTester:
    def __init__(self):
        ...
        self.detector = ShootingStarPattern()
        self.results = []
        print(f"ShootingStarPatternTester initialized")

    def _clear_old_charts(self):
        if self.charts_dir.exists():
            chart_files = list(self.charts_dir.glob('shooting_star_*.png'))
    ...
    def test_candle_by_candle(self, df, timeframe='5min', start_from=100, window_size=20):
        ...
        for i in range(start_from, total_candles):
            ...
            if is_detected:
                ...
                print(f"\nShooting Star #{len(detections)} detected!")
    ...
    def _plot_detection(self, df, detected_at_index, pattern_index, timeframe,
        ...
        try:
            ...
            if patterns_in_range:
                for det in patterns_in_range:
                    ...
                    if det_idx == pattern_index:
                        ...
                        ax.scatter([position], [candle['high']], color='orange', s=150, marker='v', zorder=4,
            ...
            ax.set_title(
                f'Shooting Star Pattern Detection - BTC/USDT {timeframe}\n'
            ...
            filename = f"shooting_star_{timeframe}_detect{detected_at_index}_pattern{pattern_index}_{timestamp_str}.png"
    ...
    def _plot_all_patterns(self, df, pattern_indices, timeframe):
        ...
        try:
            ...
            ax.set_title(
                f'All Shooting Star Patterns - BTC/USDT {timeframe}\n'
            ...
            ax.scatter([], [], color='red', s=150, marker='v',
                      label=f'Shooting Star Pattern ({len(pattern_indices)} total)')
            ...
            filename = f"shooting_star_{timeframe}_ALL_PATTERNS_summary.png"
            filepath = self.charts_dir / filename
    ...
    def _save_results(self, timeframe):
        results_file = self.output_dir / f'shooting_star_detections_{timeframe}.json'
...
def main():
    print("="*80)
    print("Shooting Star Pattern Test")
    print("="*80)

    try:
        tester = ShootingStarPatternTester()
