"""
Spinning Top Pattern Test on Historical BTC Data - Windows Version
...
try:
    ...
    print("SpinningTopPattern imported successfully")
except ImportError as e:
    print(f"Error importing SpinningTopPattern: {e}")
    sys.exit(1)
...
class SpinningTopPatternTester:
    def __init__(self):
        ...
        self.detector = SpinningTopPattern()
        self.results = []
        print(f"SpinningTopPatternTester initialized")

    def _clear_old_charts(self):
        if self.charts_dir.exists():
            chart_files = list(self.charts_dir.glob('spinning_top_*.png'))
    ...
    def test_candle_by_candle(self, df, timeframe='5min', start_from=100, window_size=20):
        ...
        for i in range(start_from, total_candles):
            ...
            if is_detected:
                ...
                print(f"\nSpinning Top #{len(detections)} detected!")
    ...
    def _plot_detection(self, df, detected_at_index, pattern_index, timeframe,
        ...
        try:
            ...
            ax.set_title(
                f'Spinning Top Pattern Detection - BTC/USDT {timeframe}\n'
            ...
            filename = f"spinning_top_{timeframe}_detect{detected_at_index}_pattern{pattern_index}_{timestamp_str}.png"
    ...
    def _plot_all_patterns(self, df, pattern_indices, timeframe):
        ...
        try:
            ...
            ax.set_title(
                f'All Spinning Top Patterns - BTC/USDT {timeframe}\n'
            ...
            ax.scatter([], [], color='orange', s=150, marker='v',
                      label=f'Spinning Top Pattern ({len(pattern_indices)} total)')
            ...
            filename = f"spinning_top_{timeframe}_ALL_PATTERNS_summary.png"
            filepath = self.charts_dir / filename
    ...
    def _save_results(self, timeframe):
        results_file = self.output_dir / f'spinning_top_detections_{timeframe}.json'
...
def main():
    print("="*80)
    print("Spinning Top Pattern Test")
    print("="*80)

    try:
        tester = SpinningTopPatternTester()
