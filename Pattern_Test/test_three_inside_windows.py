"""
Three Inside Pattern Test on Historical BTC Data - Windows Version
...
try:
    ...
    print("ThreeInsidePattern imported successfully")
except ImportError as e:
    print(f"Error importing ThreeInsidePattern: {e}")
    sys.exit(1)
...
class ThreeInsidePatternTester:
    def __init__(self):
        ...
        self.detector = ThreeInsidePattern()
        self.results = []
        print(f"ThreeInsidePatternTester initialized")

    def _clear_old_charts(self):
        if self.charts_dir.exists():
            chart_files = list(self.charts_dir.glob('three_inside_*.png'))
    ...
    def test_candle_by_candle(self, df, timeframe='5min', start_from=100, window_size=20):
        ...
        for i in range(start_from, total_candles):
            ...
            if is_detected:
                ...
                print(f"\nThree Inside #{len(detections)} detected!")
    ...
    def _plot_detection(self, df, detected_at_index, pattern_index, timeframe,
        ...
        try:
            ...
            ax.set_title(
                f'Three Inside Pattern Detection - BTC/USDT {timeframe}\n'
            ...
            filename = f"three_inside_{timeframe}_detect{detected_at_index}_pattern{pattern_index}_{timestamp_str}.png"
    ...
    def _plot_all_patterns(self, df, pattern_indices, timeframe):
        ...
        try:
            ...
            ax.set_title(
                f'All Three Inside Patterns - BTC/USDT {timeframe}\n'
            ...
            ax.scatter([], [], color='purple', s=150, marker='v',
                      label=f'Three Inside Pattern ({len(pattern_indices)} total)')
            ...
            filename = f"three_inside_{timeframe}_ALL_PATTERNS_summary.png"
            filepath = self.charts_dir / filename
    ...
    def _save_results(self, timeframe):
        results_file = self.output_dir / f'three_inside_detections_{timeframe}.json'
...
def main():
    print("="*80)
    print("Three Inside Pattern Test")
    print("="*80)

    try:
        tester = ThreeInsidePatternTester()
