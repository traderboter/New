"""
Three Methods Pattern Test on Historical BTC Data - Windows Version
...
try:
    ...
    print("ThreeMethodsPattern imported successfully")
except ImportError as e:
    print(f"Error importing ThreeMethodsPattern: {e}")
    sys.exit(1)
...
class ThreeMethodsPatternTester:
    def __init__(self):
        ...
        self.detector = ThreeMethodsPattern()
        self.results = []
        print(f"ThreeMethodsPatternTester initialized")

    def _clear_old_charts(self):
        if self.charts_dir.exists():
            chart_files = list(self.charts_dir.glob('three_methods_*.png'))
    ...
    def test_candle_by_candle(self, df, timeframe='5min', start_from=100, window_size=20):
        ...
        print(f"Lookback: 12, Min candles: 17")
        ...
        for i in range(start_from, total_candles):
            ...
            if is_detected:
                ...
                print(f"\nThree Methods #{len(detections)} detected!")
    ...
    def _plot_detection(self, df, detected_at_index, pattern_index, timeframe,
        ...
        try:
            ...
            ax.set_title(
                f'Three Methods Pattern Detection - BTC/USDT {timeframe}\n'
            ...
            filename = f"three_methods_{timeframe}_detect{detected_at_index}_pattern{pattern_index}_{timestamp_str}.png"
    ...
    def _plot_all_patterns(self, df, pattern_indices, timeframe):
        ...
        try:
            ...
            ax.set_title(
                f'All Three Methods Patterns - BTC/USDT {timeframe}\n'
            ...
            ax.scatter([], [], color='purple', s=150, marker='v',
                      label=f'Three Methods Pattern ({len(pattern_indices)} total)')
            ...
            filename = f"three_methods_{timeframe}_ALL_PATTERNS_summary.png"
            filepath = self.charts_dir / filename
    ...
    def _save_results(self, timeframe):
        results_file = self.output_dir / f'three_methods_detections_{timeframe}.json'
...
def main():
    print("="*80)
    print("Three Methods Pattern Test")
    print("="*80)

    try:
        tester = ThreeMethodsPatternTester()
