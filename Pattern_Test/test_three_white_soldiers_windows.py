"""
Three White Soldiers Pattern Test on Historical BTC Data - Windows Version
...
try:
    ...
    print("ThreeWhiteSoldiersPattern imported successfully")
except ImportError as e:
    print(f"Error importing ThreeWhiteSoldiersPattern: {e}")
    sys.exit(1)
...
class ThreeWhiteSoldiersPatternTester:
    def __init__(self):
        ...
        self.detector = ThreeWhiteSoldiersPattern()
        self.results = []
        print(f"ThreeWhiteSoldiersPatternTester initialized")

    def _clear_old_charts(self):
        if self.charts_dir.exists():
            chart_files = list(self.charts_dir.glob('three_white_soldiers_*.png'))
    ...
    def test_candle_by_candle(self, df, timeframe='5min', start_from=100, window_size=20):
        ...
        print(f"Pattern: 3-candle bullish")
        ...
        for i in range(start_from, total_candles):
            ...
            if is_detected:
                ...
                print(f"\nThree White Soldiers #{len(detections)} detected!")
    ...
    def _plot_detection(self, df, detected_at_index, pattern_index, timeframe,
        ...
        try:
            ...
            ax.set_title(
                f'Three White Soldiers Pattern Detection - BTC/USDT {timeframe}\n'
            ...
            filename = f"three_white_soldiers_{timeframe}_detect{detected_at_index}_pattern{pattern_index}_{timestamp_str}.png"
    ...
    def _plot_all_patterns(self, df, pattern_indices, timeframe):
        ...
        try:
            ...
            ax.set_title(
                f'All Three White Soldiers Patterns - BTC/USDT {timeframe}\n'
            ...
            ax.scatter([], [], color='blue', s=150, marker='v',
                      label=f'Three White Soldiers Pattern ({len(pattern_indices)} total)')
            ...
            filename = f"three_white_soldiers_{timeframe}_ALL_PATTERNS_summary.png"
            filepath = self.charts_dir / filename
    ...
    def _save_results(self, timeframe):
        results_file = self.output_dir / f'three_white_soldiers_detections_{timeframe}.json'
...
def main():
    print("="*80)
    print("Three White Soldiers Pattern Test")
    print("="*80)

    try:
        tester = ThreeWhiteSoldiersPatternTester()
