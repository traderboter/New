...
from signal_generation.shared.data_models import TradeResult
...
from signal_generation.systems.adaptive_learning_system import AdaptiveLearningSystem
...
class CryptoTradingBot:
    ...
    async def initialize_components(self):
        ...
        try:
            ...
            # 2.5. راه‌اندازی IndicatorCalculator
            self.indicator_calculator = IndicatorCalculator(self.config)
            logger.info("محاسبه‌گر اندیکاتورها راه‌اندازی شد")
                ...
                def update_signal_generator_config(new_config):
                    ...
                    if 'signal_generation' in new_config:
                        ...
                        if hasattr(self.signal_generator, 'minimum_signal_score'):
                            self.signal_generator.minimum_signal_score = signal_config.get('minimum_signal_score', 50)
    ...
    async def run(self):
        ...
        try:
            ...
            # بررسی فعال بودن آموزش اولیه از config
            ...
            elif self.trading_brain and not initial_training_enabled:
                logger.info("آموزش اولیه مدل‌های هوش مصنوعی در تنظیمات غیرفعال است (initial_training_enabled=False)")
