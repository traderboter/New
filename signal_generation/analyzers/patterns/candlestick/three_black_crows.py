...
- 🔄 Detection now checks last 15 candles by default (not just current)
- 📊 Score adjusts based on pattern age (0-15 candles ago)
- 🔬 Based on research: min 16 candles required (15 lookback + 1 current)
"""

THREE_BLACK_CROWS_PATTERN_VERSION = "3.0.0"
...
class ThreeBlackCrowsPattern(BasePattern):
    ...
    def detect(
        ...
        """
        Detect Three Black Crows pattern in last N candles using TALib.
        ...
        Based on research:
        - Minimum 16 candles required (15 lookback + 1 current)
        """
        ...
        # Based on research: need minimum 16 candles
        if len(df) < 16:
            return False
    ...
    def _get_detection_details(self, df: pd.DataFrame) -> Dict[str, Any]:
        """
        Get additional details about Three Black Crows detection with recency information.
